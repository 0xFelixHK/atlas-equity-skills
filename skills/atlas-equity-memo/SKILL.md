...
Use the Atlas framework modules as lenses when they fit the company or user request:

- `atlas-alpha`: when the thesis depends on news, product launches, procurement signals, supply-chain changes, or market misclassification. Translate news into observable demand and financial-statement impact.
- `atlas-growth-valuation`: when the key question is whether intrinsic 3-5 year growth probability is above or below market-implied growth.
- `atlas-tam-peg`: when valuation depends on high growth, TAM runway, growth duration, and business quality rather than a simple PEG.
- `atlas-market-health`: when the user asks whether the current trend or entry point is healthy, or when price/DMA/ATR and estimate-revision data are available.
- `atlas-cycle-stage`: when the thesis depends on industry capex, ASP, inventory, supply-demand, capacity release, customer order behavior, or migration across recovery, expansion, overheating, downturn, and clearing phases.
